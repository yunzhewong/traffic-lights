...
#define MIN_DURATION 3   // Needs to be greater >= than pedestrian cutoff
...
enum TrafficState { Red, Pedestrian, Green, Yellow, Error };
...
struct transition_state_t {
    ...
    void transition_after_duration(uint8_t duration_s, TrafficState target_state);
};
...
struct traffic_state_t {
    traffic_state_t(DirectionalLights* north_south_direction, DirectionalLights* east_west_direction,
