...
uint8_t catch_message(USBConnection &usb_connection, uint8_t *read_buffer,
  ...
  uint8_t read_amount = usb_connection.read_with_reset(&read_buffer[read_count],
                                                       COMMS_SIZE - read_count);
  ...
  if (read_count > 0 && read_buffer[0] != DELIMITER) {
    ...
    while (delimiter_index < read_count &&
           read_buffer[delimiter_index] != DELIMITER) {
      delimiter_index++;
    }
    // downshift
