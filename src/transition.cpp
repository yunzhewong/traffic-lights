...
void transition_state_t::transition_after_duration(uint8_t duration_s, TrafficState target_state) {
    ...
    this->transition_ticks = (uint8_t)((double)duration_s / TICK_PERIOD);  // this is will fit because duration_s is
                                                                           // less than MAX_PERIOD (20s)
    if (time_since_transition_s > duration_s) {
        this->state_enum = target_state;
        this->transition_time_us = current_time_us;
    }
}
traffic_state_t::traffic_state_t(DirectionalLights* north_south_direction, DirectionalLights* east_west_direction,
    ...
    : north_south_direction(north_south_direction),
      ...
      state_references(
          {north_south_direction, east_west_direction, &pedestrian_request->east, &pedestrian_request->west}),
...
void traffic_state_t::handle_transition() {
    switch (transition_state.state_enum) {
        ...
        case Green: {
            ...
            state_references.secondary->set_red();
            transition_state.transition_after_duration(*this->green_duration, TrafficState::Yellow);
    ...
    bool lights_valid = validate_lights(*this->north_south_direction, *this->east_west_direction);

    if (!lights_valid) {
        transition_state.state_enum = TrafficState::Error;
    }
}
void traffic_state_t::change_green_durations(uint8_t north_south, uint8_t east_west) {
...
packed_state_t traffic_state_t::pack_state() {
    ...
    return packed_state_t{traffic_byte, pedestrian_byte, request_byte, this->transition_state.current_ticks,
                          this->transition_state.transition_ticks};
}
uint8_t traffic_state_t::pack_traffic() {
    uint8_t north_south_contribution = north_south_direction->traffic.get_byte_state() << 4;
    uint8_t east_west_contribution = east_west_direction->traffic.get_byte_state();
...
uint8_t traffic_state_t::pack_pedestrian() {
    ...
    return north_contribution + east_contribution + south_contribution + west_contribution;
}
